...
bool Peon::esmovimientoValido(int x_Destino, int y_Destino, int comer) {
	...
	if (comer >= 1) { // Solo si va a comer
		...
		if ((color == 1) && (((x_Destino - getX()) == 1 && (y_Destino - getY()) == 1)||(comer == 3))) { // Superior derecha
			return 1;
		}
		else if ((color == 1) && (((x_Destino - getX()) == -1 && (y_Destino - getY()) == 1) || (comer == 2))) { // Superior izquierda
			return 1;
		}
		else if ((color == 0) && (((x_Destino - getX()) == -1 && (y_Destino - getY()) == -1) || (comer == 3))) { // inferior izquierda
			return 1;
		}
		else if ((color == 0) && (((x_Destino - getX()) == 1 && (y_Destino - getY()) == -1) || (comer == 2))) { // inferior derecha
			return 1;
		}
		else { return 0; }
	}
